...
public class ActivityPackageCommandService {
    ...
    // 새 활동 패키지를 생성하고 응답 DTO를 반환한다.
    public ActivityPackageResponse createPackage(ActivityPackageCreateRequest request, Long userId) {
        ActivityPackage activityPackage = activityPackageRepository.save(request.toEntity(userId));
        return ActivityPackageResponse.from(activityPackage);
    }

    // 패키지 기본 정보와 viewer/item 목록을 한 번의 조회로 모두 수정한다.
    public ActivityPackageResponse updateAll(Long packageId, ActivityPackageUpdateRequest request) {
