...
public class ContactQueryController {
    ...
    public ResponseEntity<CollectionModel<EntityModel<ContactResponse>>> getContacts(
        ...
        List<EntityModel<ContactResponse>> models = contacts.stream()
                .map(c -> EntityModel.of(c,
                        linkTo(methodOn(ContactQueryController.class).getContacts(clientId)).withSelfRel()))
                .toList();
    ...
    public ResponseEntity<CollectionModel<EntityModel<ContactResponse>>> getContactsByClientId(@PathVariable Long clientId) {
        List<ContactResponse> contacts = contactQueryService.getContactsByClientId(clientId)
                .stream().map(ContactResponse::from).toList();

        List<EntityModel<ContactResponse>> models = contacts.stream()
                .map(c -> EntityModel.of(c,
                        linkTo(methodOn(ContactQueryController.class).getContactsByClientId(clientId)).withSelfRel()))
                .toList();
